...

{
    ...
    if (IsEqualGUID (Pin -> ConnectionFormat -> Specifier,
        ...
        //
            ...
            (ConnectionFormat -> VideoInfoHeader.bmiHeader.biWidth < 160) ||
            (ConnectionFormat -> VideoInfoHeader.bmiHeader.biWidth > 3840) ||
            (abs(ConnectionFormat -> VideoInfoHeader.bmiHeader.biHeight) < 120) ||
            (abs(ConnectionFormat -> VideoInfoHeader.bmiHeader.biHeight) > 3840) ||
            ((ConnectionFormat -> VideoInfoHeader.bmiHeader.biWidth & 1) != 0) ||
...

{
    ...
    if (ProcessPin -> BytesAvailable) {
        ...
        CamPlayerCopyFrame(
            reinterpret_cast<PUCHAR>(ProcessPin -> Data),
            m_VideoInfoHeader -> bmiHeader.biSizeImage,
            m_VideoInfoHeader -> bmiHeader.biWidth,
            m_VideoInfoHeader -> bmiHeader.biHeight,
            m_VideoInfoHeader -> bmiHeader.biBitCount,
            m_VideoInfoHeader -> bmiHeader.biCompression);
...
FormatRGB24Bpp_Capture = {
    ...
    //
    {
        ...
        3840,3840,      // InputSize, largest source accepted from Player
                        //             with every digitized pixel unique)
        160,120,        // MinCroppingSize, smallest rcSrc cropping rect allowed
        3840,3840,      // MaxCroppingSize, largest rcSrc cropping rect allowed
        ...
        3840, 3840,     // MaxOutputSize, largest bitmap stream can produce
